...
const STATUS_META = {
  ON_TIME: {
    label: "On time",
    classes: "bg-volt-500/10 text-volt-500 border-volt-500/30",
  },
  LATE: {
    label: "Late",
  ...
  EARLY_LEAVE: {
    label: "Early leave",
    classes: "bg-ember-500/10 text-ember-500 border-ember-500/30",
  },
  ACTIVE: {
    label: "Still in",
...
function StatusBadge({ status }) {
  const meta = STATUS_META[status] || {
    label: status || "—",
    classes: "bg-ink-700 text-ink-300 border-ink-600",
  };
  return (
    <span
      className={`inline-flex items-center rounded-full border px-2.5 py-0.5 text-[11px] font-semibold whitespace-nowrap ${meta.classes}`}
    >
      {meta.label}
    </span>
  );
...
export default function AttendanceAdmin() {
  ...
  usePageMeta(
    isBranchAdmin ? "Attendance · Branch" : "Attendance · Admin",
    isBranchAdmin
      ? "Attendance for your branch members"
      : "All members, check-in and check-out at a glance",
  );
  ...
  const load = async () => {
    ...
    try {
      ...
      // Only ADMIN can select branch manually
      if (isAdmin && branchId) {
        attendanceParams.branch_id = branchId;
      }

      // Branch Admin is always restricted to own branch
      if (isBranchAdmin) {
        attendanceParams.branch_id = user.branch_id;
      }

      const [attRes, branchRes, memberRes] = await Promise.all([
        attendanceApi.history(attendanceParams),

        isAdmin ? branchApi.list() : Promise.resolve({ data: [] }),
  ...
  const filteredRows = useMemo(() => {
    ...
    return rows.filter((r) => {
      ...
      if (
        statusFilter &&
        r.checkInStatus !== statusFilter &&
        r.checkOutStatus !== statusFilter
      )
        return false;
      if (q) {
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
        {STAT_CARDS.map(({ key, label, icon: Icon, classes }) => (
          <div key={key} className="surface-card p-4 flex items-center gap-3">
            ...
            <div>
              <div className={`font-display text-lg font-bold ${classes}`}>
                {stats[key]}
              </div>
