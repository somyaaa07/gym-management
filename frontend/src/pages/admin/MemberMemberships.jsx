...
export default function MemberMemberships() {
  ...
  const onEditSubmit = async (e) => {
    ...
    setEditSaving(true);
    try {
      await memberMembershipApi.update(editTarget.id, {
        ...editForm,
  ...
  return (
    <div className="space-y-6">
      <p className="text-sm text-ink-400">
        {rows.length} membership{rows.length !== 1 ? "s" : ""} · enroll a member
          ...
          columns={[
            {
              ...
              render: (r) => (
                <Link
                  to={`/app/members/${r.Member?.id || r.member_id}`}
                  className="hover:text-volt-500"
                ...
                <div className="flex items-center gap-1 justify-end">
                  ...
                  {r.status !== "FROZEN" && r.status !== "DEACTIVE" && (
                    <button
                      ...
                      className="p-1.5 text-ink-400 hover:text-sky-400 rounded-xl hover:bg-ink-700"
                    >
                      <Snowflake size={14} />
                    </button>
                  )}
      ...
      <Modal
        ...
        subtitle={editTarget?.Member?.name}
        width="max-w-sm"
      >
        <form onSubmit={onEditSubmit} className="space-y-4">
          ...
          <Field label="Auto-renew">
            <Select
              value={editForm.auto_renew ? "yes" : "no"}
              onChange={(e) =>
                setEditForm({
                  ...editForm,
                  auto_renew: e.target.value === "yes",
                })
              }
            >
      ...
      <Modal
        ...
        subtitle="Extends the end date by the frozen days"
        width="max-w-sm"
      >
        <form onSubmit={onFreeze} className="space-y-4">
          <Field label="Freeze from" required>
            <Input
              ...
              onChange={(e) =>
                setFreezeForm({
                  ...freezeForm,
                  freeze_start_date: e.target.value,
                })
              }
            />
          </Field>
          <Field label="Freeze until" required>
            <Input
              ...
              onChange={(e) =>
                setFreezeForm({
                  ...freezeForm,
                  freeze_end_date: e.target.value,
                })
              }
            />
          </Field>
          ...
          <div className="flex gap-2 pt-1">
            ...
            <Button type="submit" loading={freezeSaving} className="flex-1">
              Freeze
            </Button>
          </div>
        </form>
