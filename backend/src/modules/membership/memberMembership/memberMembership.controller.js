...
import {memberMembershipSchema,updateMemberMembershipSchema} from './memberMembership.validation.js';
...
export const updateMemberMembership = async(req,res)=>{
    try{
        ...
        const {membership_plan_id,payment_status,auto_renew,discount,freeze_start_date,freeze_end_date} = result.data
        ...
        let price = Number(membership.price);
        let endDate = membership.end_date;
        let newPlanId = membership.membership_plan_id;
        ...
        if(!membershipPlan){
            return res.status(404).json({
                success:false,
                message:"not found the selected plan is activated"
            })
        }
          ...
          //recalculated the plan
          const startDate = new Date(membership.start_date);
          endDate = new Date(membership.start_date);
        ...
        const newDiscount = discount ?? Number(membership.discount);
        const newPayment_status = payment_status ?? membership.payment_status;
        const newAutorenew = auto_renew ?? membership.auto_renew;

  
          if(newDiscount > price){
            return res.status(400).json({
                success:false,
                message:"Discount can't be bigger then the price"
            })
        }
        ...
        const newFreezeStartDate = freeze_start_date ?? membership.freeze_start_date;
        const newFreezeEndDate = freeze_end_date ?? membership.freeze_end_date;

        if(newFreezeEndDate && newFreezeStartDate){
            if(newFreezeEndDate<newFreezeStartDate){
                return res.status(400).json({
                    success:false,
                    message:"start date can not be bigger then end date"
                })
            }
        }
        ...
        console.log("failed in updating the member's membership plan ",err)
        return res.status(500).json({
            success:false,
            message:"Internal server error"
        })
