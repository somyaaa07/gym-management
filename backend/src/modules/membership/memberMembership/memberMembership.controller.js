...
import {
  ...
  freezeMemberMembershipSchema,
} from "./memberMembership.validation.js";
...
export const getAllMemberships = async (req, res) => {
  ...
  try {
    ...
    // branch admin => only members of their branch
    if (req.user.role === "BRANCH_ADMIN") {
      memberWhere.branch_id = req.user.branch_id;
    }
...
export const updateMemberMembership = async (req, res) => {
  try {
    ...
    const newDiscount =
      discount !== undefined ? Number(discount) : Number(membership.discount);

    const newPaymentStatus =
      payment_status !== undefined ? payment_status : membership.payment_status;

    const newAutoRenew =
      auto_renew !== undefined ? auto_renew : membership.auto_renew;
    ...
    console.log("failed in updating the member's membership", err);
