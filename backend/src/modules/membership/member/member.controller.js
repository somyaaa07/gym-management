...
import { Branch, Member, MemberMembership, MembershipPlan, User } from '../../../model/index.js'
import { memberValidationSchema, updateMemberValidationSchema ,setPasswordValidationSchema} from './memberValidation.js';
...
export const createMember = async (req, res) => {
    ...
    try {
        ...
        const user = await User.create({
            name, email, phone, tenant_id, branch_id,
            password: null,
            role: "MEMBER",
...
export const setPassword = async (req, res) => {
    try {
        ...
        const user = await User.findOne({
            where: { reset_token: hashedToken, reset_token_expiry: { [Op.gt]: new Date() } }
        });
        ...
        return res.status(200).json({ success: true, message: "Password set successfully. You can now login." })
    }
    catch (err) {
